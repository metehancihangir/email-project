...
const faqs = [
  ...
  },
  {
    ...
    answer: "Tıpkı abonelikten çıkma işlemi gibi, size gelen e-postaların alt kısmında yer alan \"Tercihleri Güncelle\" bağlantısına tıklayarak ilgili sayfaya gidebilirsiniz. O sayfadan (örneğin Finans, Bilim veya Mitoloji arasından) almak istediğiniz bültenleri dilediğiniz gibi açıp kapatabilirsiniz."
  },
  {
        ...
        <div className="rounded-2xl p-4 md:p-8">
          ...
          <div className="relative mb-8">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all text-text bg-white shadow-sm"
            />
          </div>
          ...
          <div className="space-y-4">
            {filteredFaqs.length > 0 ? (
              filteredFaqs.map((faq, index) => (
                <div key={index} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden transition-all duration-200">
                  <button
                    onClick={() => toggleAccordion(index)}
                    className="w-full px-6 py-4 flex items-center justify-between focus:outline-none hover:bg-gray-50 transition-colors"
                  >
                    <span className="font-semibold text-text text-left drop-shadow-sm">{faq.question}</span>
                    <svg 
                  ...
                  <AnimatePresence>
                    {openIndex === index && (
                      ...
                      >
                        <div className="px-6 pb-4 pt-2 text-text-muted">
                          {faq.answer}
                        </div>
                      </motion.div>
                    )}
